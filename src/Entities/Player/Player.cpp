...
void Player::update() {
    ...
    if (HitBox::Collision(Background::sideWalls.first, this->hitBox)) {
        std::pair<double,double> collision =
            HitBox::CollisionMargins(this->hitBox, Background::sideWalls.first);
    ...
    if (HitBox::Collision(Background::sideWalls.second, this->hitBox)) {
        std::pair<double,double> collision =
            HitBox::CollisionMargins(this->hitBox, Background::sideWalls.second);
...
void Player::attack() {
    if (cooldown <= 0) {
        ...
        Projectile::projectiles.push_back(
            Projectile(
                position.first + this->hitBox.box.width / 2,
                position.second,
                0
            )
        );
